class Book {
  ...
  display() {
    this.bookArray.forEach(([key, value]) => {
      this.html.push(`<div class='book d-flex col-12 p-2'><div class='d-flex col-10 text-container'><p class='book-title m-0'>${key} by</p><p class='book-author m-0'>&nbsp;${value}</p></div><button class='remove' onclick="book.removeBooks('${key}')">Remove</button></div>`);
    });
